...
- [ ] Dockerfiles (`src/Clutch.Server/Dockerfile`, `dashboard/Dockerfile`) exist to **build and push** `jchristn77/clutch-server:v0.1.0` and `jchristn77/clutch-ui:v0.1.0`; provide root `build-all.bat`/`build-server.bat`/`build-dashboard.bat` that take the tag as an argument and build+tag (given tag + `latest`)+push multi-platform on the Docker Build Cloud builder. `compose.yaml` only references the published tags — never a build context.
...
- [ ] Since `compose.yaml` references published tags, `docker/factory/templates/` includes the pinned `compose.yaml`; local iteration on server/dashboard code requires a rebuild+push via the root build scripts (there is no `--build` override compose file — compose never builds from a context).
