...
The REST API handles administration and observability: tokens, tenants, users, application keys, lock inspection, audit, request history, and server info. Lock acquisition itself is over the [WebSocket API](WEBSOCKETS_API.md).
...
Lock acquisition is over the WebSocket; these routes inspect and administer live state.
...
- **`POST /v1.0/api/tenants/{tid}/locks/{key}/release`** — tenant admin. Force-releases every holder on the key. Returns `{ key, released }`.
